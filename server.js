...
const stats = { plays: 0, totalMs: 0, completed: 0, finished: 0, first: Date.now(), lastPlayed: 0, days: {} };
try { Object.assign(stats, JSON.parse(await readFile(STATS_FILE, "utf8"))); } catch { /* first run */ }
if (stats.finished === undefined) stats.finished = 0; // migrate older stats files
if (stats.lastPlayed === undefined) stats.lastPlayed = 0;
let _saveT = null;
...
function crmPage() {
  ...
  const rows = days.map(([d, v]) => `<tr><td>${d}</td><td>${v.plays || 0}</td><td>${v.finished || 0}</td><td>${v.completed ? fmtDur(v.ms / v.completed) : "—"}</td></tr>`).join("") || `<tr><td colspan="4" style="opacity:.5">no data yet</td></tr>`;
...
<div class="cards">${card("Plays", stats.plays)}${card("Games finished", finished)}${card("Avg play time", stats.completed ? fmtDur(avg) : "—")}${card("Total play time", fmtDur(stats.totalMs))}</div>
<p class="last" id="lastplayed" data-ts="${stats.lastPlayed || 0}">Last played: —</p>
<script>(function(){var el=document.getElementById('lastplayed');var ts=+el.dataset.ts;el.textContent='Last played: '+(ts?new Date(ts).toLocaleString(undefined,{weekday:'short',year:'numeric',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}):'—');})();</script>
<h2>Last 14 days</h2><table><thead><tr><th>Day</th><th>Plays</th><th>Finished</th><th>Avg time</th></tr></thead><tbody>${rows}</tbody></table>
<p class="foot">A "play" = a session that reached gameplay. "Games finished" = sessions where the player actually beat the mission (recovered all Arcs). Avg/total play time covers every session we got a duration for (win, loss, or leaving).</p></div></body></html>`;
}
const MIME = {
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (p === "/api/track" && req.method === "POST") {
      ...
      const d = stats.days[day] || (stats.days[day] = { plays: 0, completed: 0, finished: 0, ms: 0 });
      if (d.finished === undefined) d.finished = 0;
      if (j.type === "start") { stats.plays++; d.plays++; stats.lastPlayed = Date.now(); saveStats(); }
      else if (j.type === "end") {
        const ms = Math.max(0, Math.min(6 * 3600 * 1000, +j.ms || 0));
        if (ms > 1500) { stats.totalMs += ms; stats.completed++; d.completed++; d.ms += ms; } // any session with a real duration → play-time stats
        if (j.finished) { stats.finished = (stats.finished || 0) + 1; d.finished++; } // only an actual win counts as a finished game
